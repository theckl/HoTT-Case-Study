...
@[hott]
def subtype_emb {C : Category} (P : C -> trunctype.{0} -1) :
  subtype (λ c : C, ↥(P c)) -> C := assume sc, sc.1

@[hott, instance]
def subtype_emb_is_inj {C : Category} (P : C -> trunctype.{0} -1) :
  is_injective (subtype_emb P) :=
begin intros sc₁ sc₂, exact (subtype_eq_equiv sc₁ sc₂).to_is_equiv end    

@[hott, instance]
def full_subcat_on_subtype {C : Category} (P : C -> trunctype.{0} -1) :
  is_cat (subtype (λ c : C, ↥(P c))) :=
@fully_embedded_category _ _ (subtype_emb P) (subtype_emb_is_inj P)  

@[hott]
def embed {C : Category} {P : C -> trunctype.{0} -1} 
  {D : Precategory} (F : D ⥤ subtype (λ c : C, ↥(P c))) : D ⥤ C :=
have G : subtype (λ c : C, ↥(P c)) ⥤ C, from emb_functor (subtype_emb P), 
F ⋙ G 
