...
@[hott, class] 
structure partial_order (α : Type _) extends has_le α :=
  (le_refl : Πa, le a a)
  (le_trans : Πa b c, le a b → le b c → le a c)
  (le_antisymm : Πa b, le a b → le b a → a = b)

@[hott]
def is_po_unit {α : Type _} [partial_order α] (one : α) := 
  Π a, a ≤ one 

@[hott]
def po_unit_is_unique {α : Type _} [po : partial_order α] : 
  Π a a' : α, is_po_unit a -> is_po_unit a' -> a = a' :=
